...
class GaussianPointCloudScene(torch.nn.Module):
    ...
    def __init__(
        ...
        self._instance = nn.Parameter(torch.zeros((point_cloud.shape[0], self.num_categories), dtype=torch.float, device="cuda"))
    ...
    def to_parquet(self, path: str):
        ...
        valid_point_instance = self._instance[self.point_invalid_mask == 0]
        ...
        point_instance_df = pd.DataFrame(
            valid_point_instance.detach().cpu().numpy(), columns=[f"instance_{i}" for i in range(self.num_categories)])
        scene_df = pd.concat([point_cloud_df, point_cloud_features_df,point_instance_df], axis=1)
    ...
    def from_parquet(path: str, config=PointCloudSceneConfig()):
        ...
        # instance = scene_df[["instance"]].to_numpy()
